...
public class TrBarrageHitSoundPacket {
    ...
    public TrBarrageHitSoundPacket(int standEntityId, SoundEvent sound, Vector3d soundPos) {
        this(standEntityId, true, false, sound, soundPos);
    }
    
    public static TrBarrageHitSoundPacket noSound(int standEntityId) {
        return new TrBarrageHitSoundPacket(standEntityId, false, false, null, null);
    }
    
    public static TrBarrageHitSoundPacket barrageStopped(int standEntityId) {
        return new TrBarrageHitSoundPacket(standEntityId, false, true, null, null);
    }
    
    private TrBarrageHitSoundPacket(int standEntityId, boolean hit, boolean stop, SoundEvent sound, Vector3d soundPos) {
    ...
    public static TrBarrageHitSoundPacket decode(PacketBuffer buf) {
        ...
        if (buf.readBoolean()) {
            return TrBarrageHitSoundPacket.barrageStopped(entityId);
        }
        if (buf.readBoolean()) {
    ...
    public static void handle(TrBarrageHitSoundPacket msg, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            if (entity instanceof StandEntity) {
                ...
                if (msg.stop) {
                    stand.getBarrageHitSoundsHandler().setIsBarraging(false);
                }
                else {
                    if (msg.hit) {
